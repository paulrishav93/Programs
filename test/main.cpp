...
private:
    ...
    string player_name;
    vector< ii > empty_points, points_to_be_removed;            //deals with empty points on board in simulation
    ...
    int first_player,present_player,no_of_empty_spaces;                            //keeps info about the present and the first player
    int *empty_points_shuffle;
    ...
    inline int row_val(int);
    inline int column_val(int);
...
HexBoard::HexBoard(int size):
    ...
    NORTH(size*size+3),SOUTH(size*size+4),marked_bfs(vector<int>(size*size+5,-1))
    {
        ...
        empty_points_shuffle=new int[SIZE*SIZE+1];
        start_game();
    }
...
void HexBoard::input_from_computer(int &row, int &column)
{
    ...
    no_of_empty_spaces=find_empty_points();
...
int HexBoard::do_simulation()
{
    ...
    {
        random_shuffle(empty_points_shuffle + 1, empty_points_shuffle + n + 1);

        auto eps_itr=1;
        int count_space=1;

        //decides which points should be actuall8y occupied by the computer
        //in the simulation
        while(count_space<=spaces_to_fill && eps_itr<= n)
        {
            x1_pos=row_val(empty_points_shuffle[eps_itr]);
            y1_pos=column_val(empty_points_shuffle[eps_itr]);

            if(x1_pos!=x_pos || y1_pos != y_pos && board[x1_pos][y1_pos] == '.')
            {
                points_filled_by_simulation.push_back(ii (x1_pos, y1_pos));
                count_space++;
        ...
        {
            vertices[pfbs_itr->first][pfbs_itr->second].second=player_color::BLACK;
        }
...
//on the HexBoard
int HexBoard::find_empty_points()
{
    int k=0;
    for(auto i=1;i<=SIZE;++i)
        for(auto j=1;j<=SIZE;++j)
            if(board[i][j]=='.')
            {
                ...
                empty_points_shuffle[k++]=i*(SIZE-1)+j;
            }
    return k;
}
